...
| `MULTICA_DAEMON_RUNTIME_MCP` | `true` | Multica가 관리하는 `mcp_config`를 가진 에이전트가 이 호스트 자체의 MCP 서버(`~/.claude.json`, `~/.cursor/mcp.json`, `$CODEX_HOME/config.toml` 등)까지 상속할지 여부. 개인 MCP 설정을 모든 실행에 넘길 이유가 없는 머신에서는 `false`로 설정하세요. 각 에이전트는 자신의 설정이 선언한 서버만 보게 되며, 모델 컨텍스트에 실리는 도구 스키마도 줄어듭니다. 관리되는 `mcp_config`가 없는 에이전트는 영향을 받지 않고 런타임 고유의 상속을 유지합니다 |
