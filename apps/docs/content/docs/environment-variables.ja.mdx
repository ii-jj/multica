...
| `MULTICA_DAEMON_RUNTIME_MCP` | `true` | Multica が管理する `mcp_config` を持つエージェントが、このホスト自身の MCP サーバー（`~/.claude.json`、`~/.cursor/mcp.json`、`$CODEX_HOME/config.toml` など）も継承するかどうか。個人の MCP 設定をすべての実行に渡すべきでないマシンでは `false` にします。各エージェントは自身の設定が宣言したサーバーだけを見るようになり、モデルのコンテキストに載るツールスキーマも減ります。管理された `mcp_config` を持たないエージェントは影響を受けず、ランタイム本来の継承のままです |
