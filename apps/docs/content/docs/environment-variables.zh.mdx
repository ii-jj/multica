...
| `MULTICA_DAEMON_RUNTIME_MCP` | `true` | 带有 Multica 托管 `mcp_config` 的 Agent 是否同时继承本机自己的 MCP server（`~/.claude.json`、`~/.cursor/mcp.json`、`$CODEX_HOME/config.toml` 等）。若本机的个人 MCP 配置不该暴露给每次运行，设为 `false`：此时每个 Agent 只能看到自身配置声明的 server，模型上下文里的工具 schema 也随之减少。未配置托管 `mcp_config` 的 Agent 不受影响，仍走运行时原生的继承逻辑 |
