...
| `MULTICA_DAEMON_RUNTIME_MCP` | `true` | Whether agents that carry a Multica-managed `mcp_config` also inherit this host's own MCP servers (`~/.claude.json`, `~/.cursor/mcp.json`, `$CODEX_HOME/config.toml`, ...). Set to `false` on a machine whose personal MCP configuration is not every run's business: each agent then sees exactly the servers its own configuration declares, which is also fewer tool schemas in the model's context. Agents without a managed `mcp_config` are unaffected — they keep the runtime's native inheritance |
