...
func writeAvailableCommands(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("### Core\n")
	b.WriteString("- `multica issue get <id> [--compact] --output json` — full issue. `--compact` drops board bookkeeping, null fields and attachment render URLs; the description, metadata and every identity field pass through.\n")
